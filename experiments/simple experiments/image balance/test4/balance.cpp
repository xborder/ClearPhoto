...
vector<Point> divideRegions(Mat& image);
...
vector<Point> generateClusters(Mat& image, Mat& clustered, Mat& mask);
...
void ImageBalance::getBalance(const Mat& image) {
  ...
  centers = generateClusters(hsv[0], clustered, mask);
...
//  map<int,int> leftCount, rightCount;
  ...
  if ( (centers[1].y - centers[0].y) == 0) { // é recta horizontal
    unsigned char *leftoutput= (unsigned char*)(leftdivision.data);
    unsigned char *rightoutput= (unsigned char*)(rightdivision.data);
    for(int x = 0; x < image.cols; x++){
      for(int y = 0; y < image.rows; y++){
...
void drawText(Mat& image, int sum, string text, int x, int y) {
  ...
  putText(image, s.c_str(), Point(x,y), FONT_HERSHEY_DUPLEX, 0.5, Scalar::all(255));
}
...
vector<Point> generateClusters(Mat& image, Mat& clustered, Mat& mask) {
  ...
  for(int i = 0; i < subRegionWidth * subRegionHeight; i++) {
    ...
    int x = i % subRegionWidth; 
    if(mask.at<uchar>(y,x) == 255) {
      cluster_centers.at(labels.at<int>(0,counter)).x += x;
      cluster_centers.at(labels.at<int>(0,counter)).y += y;
      average_count.at(labels.at<int>(0,counter))++;
      clustered.at<float>(y,x) = (float)(colors[labels.at<int>(0,counter)]);
      counter++;
    }
  }
  ...
  for(int i = 0 ; i < cluster_centers.size(); i++) {
    ...
    circle(image, Point(point.x,point.y), 2, Scalar::all(255),5);
  }
  ...
  drawText(image, sum, " distance ", 20, 20);
  return toReturn;
}
