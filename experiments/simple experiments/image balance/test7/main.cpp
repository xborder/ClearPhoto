...
void getDivisoryMask(const Mat& image, pair<Point,Point> symmetryLine, Mat& leftMask, Mat& rightMask);
...
void sideWeight(Mat& image, Mat& canny, pair<Point, Point> symmetryLine) {
    ...
    getDivisoryMask(image, symmetryLine, leftMask, rightMask);

    countNonZeros(mask, leftMask);
    countNonZeros(mask, rightMask);
/*
    Mat tmp, blur;
    cvtColor(image, tmp, CV_BGR2GRAY);

    cv::blur( image, blur, cv::Size(5,5) );
    Canny( blur, canny, 100, 300, 3 );

    Mat leftMask, rightMask;
    getDivisoryMask(image, symmetryLine, leftMask, rightMask);

    countNonZeros(canny, leftMask);
    countNonZeros(canny, rightMask);

    Mat left = Mat::zeros(canny.size(), CV_8U);
    Mat right = Mat::zeros(canny.size(), CV_8U);
    
    bitwise_and(canny, leftMask, left);
    bitwise_and(canny, rightMask, right);
